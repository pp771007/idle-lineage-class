...
- CI 版(`.github/workflows/sync-upstream.yml`,每小時 :23 排程+可手動)做同一件事並**直推 main+發 Release(標題帶原作者版本號)**;assets 用 `rsync --delete`——所以 assets/public 絕不可放我方獨有檔。CI 已會自動更新 checkpoint(sync 腳本內建);本機跑完腳本 checkpoint 也已自動更新,不必手動改。
