...
### 🔌 「分頁進背景」＝離線開始:錨點不可再被蓋、主迴圈要停、回前景一次補完(2026-07-11 玩家回報「離線 0 分鐘、什麼都沒拿到」)

> **症狀**:手機掛機兩小時,回來顯示離線 0 分鐘、零收益;**「有機率」發生、跟版本無關**(玩家換新版仍遇到,一度被誤判成改版害的)。

三層原因疊起來,少一層都不會壞:
1. **心跳(每 5 秒)與任何 `saveGame`/`changeMap` 都會 `stamp()` 蓋離線錨點,而它們沒有 `document.hidden` 守衛**。手機鎖屏/切 App 時,頁面**常常不會被系統殺掉**、JS 只是被降速 → 錨點一路被推到「現在」→ 回來 `gap≈0`。
2. **離線結算只在 `loadGame` 觸發**。頁面沒被殺就不會重載 → 永遠不會結算。
3. 主迴圈 `gameLoop` 單次補跑上限 `MAX_CATCHUP_MS`(5 分鐘),更久的時間**直接丟掉**。

→ 結果:**只有「系統剛好把頁面殺掉」才拿得到收益**,所以玩家體感是隨機的。

解法(`js/offline.js`):
- `stamp()` 加 `document.hidden` 守衛(背景一律不蓋錨點);另開 **`stampExit()`** 給 `pagehide`/`beforeunload`/`visibilitychange(hidden)` **強制釘住離線起點**。兩者不可合併——關頁那一刻本來就是 hidden,擋掉就記不到起點。
- `visibilitychange`:**hidden → `stampExit()` + 停掉主迴圈**(背景不再邊跑邊算;否則回前景又用離線結算補同一段時間＝**同一段收益給兩次**);**visible → `gap < 5 分鐘` 交還主迴圈逐拍補完(最精確、不跳結算摘要吵人)、`gap ≥ 5 分鐘` 走 `maybeCatchup`**。
- **⚠ `maybeCatchup` 會早退(村莊/攻城/時空裂痕/木人場…不結算),此時沒有任何人重啟被我們停掉的主迴圈 → 遊戲永久卡死**。必須在 `!catchingUp` 時自己補 `startGameTimers()`(進了結算的話由 `runCatchup` 的 finally 負責重啟)。**改動「會停主迴圈」的程式碼時,每一條 return 路徑都要問:主迴圈誰負責開回來?**
- **踩雷:`startGameTimers()` 內部會 `_loopLast = null; _tickDebt = 0`**。想「把一段時間往回撥給主迴圈補跑」必須**先呼叫 `startGameTimers()`、再撥 `_loopLast`**,順序反了撥好的值會被它蓋掉 → 那段時間白白消失(改的當下就踩到,測出 tick 只前進 24 而非 1200)。

**判準:任何「會蓋離線錨點」或「會推進遊戲時間」的新程式碼,先問「分頁在背景時它還會不會跑?」**——會跑就要決定它該被 hidden 守衛擋掉,還是該由「回前景一次補完」統一負責,兩者不可並存(會重複給收益)。
