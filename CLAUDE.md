...
- **流程**:先真模擬取樣 5 分鐘(殺數 <60 自動延長至 15 分鐘)量出「平均幾拍殺一隻+每殺消耗品耗率+最低血量」→ 安全(最低血量 ≥70%、殺數 ≥8)就把剩餘時間改成「逐殺走真實獎勵管線」:`spawnMob→killMob→settleDeadMobs` 全走原作 code,**掉落擲骰/經驗/任務/卡片/收集冊/誘捕/傭兵經驗每殺獨立 RNG、與線上完全一致**;只有「殺速」與「消耗品每殺耗率」是取樣平均。實測 24h 結算從分鐘級 → **~1 秒**,金幣/經驗每小時速率 A/B 誤差 <10%(`.scratch` 曾有 test-fast-catchup/test-fast-fidelity 驗過)。
