...
> 同步成功後順手檢查一項:`afk-mobile.js` 的「renderTabs select-guard」是補原作者「戰鬥中重刷分頁 DOM 害強化下拉被關」的坑。
...
diff <(grep -v -a "afk-offline.js\|afk-mobile.js\|afk-dex.js\|可獨立維護" current_index.html) orig_index.html
```
...
- **「補原作者坑」的程式碼要標移除條件——但只標「過時後還會主動執行的」**:判準是『原作者修好後,這段會自己安靜退場,還是仍在跑?』。仍在跑的(執行期包住核心函式、長駐監聽/interval,如 `afk-mobile.js` 的 renderTabs select-guard)→ 在該段檔頭寫清楚「原作者怎麼改就能整段刪」。會自動失效的(scope 在特定選擇器的 CSS 覆寫、單純去重/防禦)→ 不必標,選擇器不命中就回原樣、留著無害。我們自己的功能(離線掛機、手機版面…)不算「補坑」,不需要這種備忘。
...
> 三者互相低耦合;手機版的離線摘要會自動打開日誌。afk-dex 純讀資料、桌機手機都掛。
