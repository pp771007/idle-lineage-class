...
3. **把新內容補進對應分頁**——分兩種:
   ...
   - **⭐ 「沒有固定來源」已自動處理,但 `gachaWeight=0` 的純兌換/試煉物品沒填 `itemAcquire` 仍是死角(這條是上面那次改動的備註)**:掉落查詢已能**自動偵測來源**——`DROPPED_SET`(怪掉,`buildIndexes` 收集)＋`_craftIndex`(製作)＋`_shopIndex`(商店)＋`itemAcquire`(手動,含兌換/試煉/喚回),由 `hasFixedSource(id)` 統一判斷。**潘朵拉限定物(`gachaWeight>0` 且 `hasFixedSource` 為 false)現在會自動被收進搜尋、且詳情卡自動補中性句「目前沒有固定取得途徑」(依規則不提潘朵拉),不必逐物品手動補 `itemAcquire`**(幻術士記憶水晶鑽石高崙/疼痛的歡愉等 7 個就是靠這個現身的)。**但「`gachaWeight=0`、只靠兌換/試煉取得、又沒填 `itemAcquire`」的物品仍搜不到**——這是刻意維持排除(避免把它誤標成沒來源),遇到這種(玩家回報某兌換/試煉物搜不到)就照上一條補一筆 `itemAcquire` 讓它現身。
