...
- **若將來要手動合併原版**:舊同步腳本從 git 歷史撈(`git log --oneline -- scripts/sync-upstream.mjs` 找刪除前 commit → `git checkout <commit>^ -- scripts/sync-upstream.mjs`)。原則=原版覆蓋 index.html/js/css → 補回全部外掛 `<script>`(帶 `?v=`)→ 補圖(走 GitHub API blob SHA,**中文檔名不可當原生 exe 參數**,git-bash 會重編碼弄壞)→ `node scripts/gen-manifests.mjs` 重產圖片對帳清單 → smoke 驗掛點。恢復自動同步同理撈回 `.github/workflows` 與 `cf-sync-trigger/`,CF Worker 需重放 GH_PAT secret 再 `wrangler deploy`。
...
## ⭐ 程式碼修改原則:功能優先「外掛 JS」,修原作者的坑可直改核心碼

> **🔓 2026-07-06 起解禁:可以直接修改原作者程式碼(`js/*.js`/`css/*.css`/`index.html`)——已停止與原版同步,不再有「被覆蓋」問題(使用者明訂)。** 適用時機:修原作者的效能/行為問題「從源頭治」比外掛包裝乾淨時,直接改(首例:`js/10-ui-tabs.js` renderTabs 分區重建+捲動守衛)。注意:①改了 `js/*.js` 要同步更新 index.html 該檔的 `?v=`(慣例=內容 sha1 前 10 碼,`sha1sum js/<檔> | cut -c1-10`)並跑 `node scripts/stamp-sw-version.mjs`;②既有外掛照舊維護,新「功能」仍偏好外掛(檔案獨立好維護),直改保留給「修原作者的坑/效能」;③改前先 grep 外掛有沒有包住同一個函式(如 afk-fixes/afk-mobile 都包過 renderTabs),避免兩層邏輯打架。

- 新功能(離線掛機、手機版面、查詢…)**一律寫成獨立的 `afk-*.js` 檔**,放專案根目錄,用 **monkey-patch / 包住全域函式 / 注入 DOM·CSS** 的方式掛上去;index.html 對外掛的引用加在 `</body>` 前。
- 外掛要能「優雅降級」:自我檢查需要的全域函式/元素是否存在,缺了就 `console.warn` 後安靜停用,**不可把遊戲弄壞**。
- **「補原作者坑」的程式碼要標移除條件——但只標「過時後還會主動執行的」**:判準是『上游修好後,這段會自己安靜退場,還是仍在跑?』。仍在跑的(執行期包住核心函式、長駐監聽/interval,如 `afk-fixes.js` 的 renderTabs select-guard)→ 放 `afk-fixes.js`(通用補坑檔)、並在該段檔頭寫清楚「上游怎麼改就能整段刪」。會自動失效的(scope 在特定選擇器的 CSS 覆寫、單純去重/防禦)→ 不必標,留著無害。我們自己的功能不算「補坑」,不需要這種備忘。(獨立維護後,這類坑也可以評估直接改核心碼根治,改完把外掛補丁刪掉。)
...
| `afk-fixes.js` | 通用修正(補上游坑、桌機/手機通用、與裝置判定無關;目前:renderTabs select-guard——戰鬥中操作強化下拉不被重繪關掉;日誌捲動錨定含 `state.ff` 快速通道) |
...
- **流程**:先真模擬取樣 5 分鐘(殺數 <60 自動延長至 15 分鐘)量出「平均幾拍殺一隻+消耗品每拍耗率+最低血量」→ 安全(最低血量 ≥門檻、殺數 ≥8;門檻預設 70%、可隨存活時間動態放寬,見下)就把剩餘時間改成「逐殺走真實獎勵管線」:`spawnMob→killMob→settleDeadMobs` 全走原作 code,**掉落擲骰/經驗/任務/卡片/收集冊/誘捕/傭兵經驗每殺獨立 RNG、與線上完全一致**;只有「殺速」與「消耗品每拍耗率」是取樣平均。實測 24h 結算從分鐘級 → **~1 秒**,金幣/經驗每小時速率 A/B 誤差 ~10%(`.scratch` 曾有 test-fast-catchup/test-fast-fidelity 驗過)。
...
- **一律退回全模擬 / 血量門檻線性歸 0(2026-07-08)**:攀登/遺忘之島/軍王之室(fastEligible 排除)、殺數太少、消耗品(箭/藥/卷軸/肉,白名單制免把「掉落被自動賣」誤判成消耗)斷貨且自動購買補不上。**升級 → 殺速變了 → 退回真模擬重取樣**(FAST_RESAMPLE_TICKS)。血量門檻**不再是固定 70%,也不是階梯放寬**:改成隨真模擬存活拍數 `done` **線性下降**——`hpFloorNow() = 70% × (1 − done/HP_FLOOR_ZERO_TICKS)`,即時用 `done` 算,取樣與 BOSS safe 共用同一條;**存活滿 20 分鐘(12000 拍)門檻歸 0**、之後一律切快速、BOSS 首遇打得贏就 safe。設計原因(使用者要求・避免整個 24h 被拖慢):真正拖慢的是**密集 BOSS 圖每種 BOSS 每次真打**(龍窟/長老之室),固定門檻下它們血量常掉破線→永遠 unsafe→整晚全逐拍;線性歸 0 讓「撐過前 20 分鐘=打得過」的角色之後全秒殺。代價=撞死即停在 20 分鐘後對快速段失效(掛在打不過的圖也秒殺收滿),但有兩層兜底:①每種 BOSS 首遇仍逐拍真打一次,那次就死照樣撞死即停;②前 20 分鐘門檻漸降、非一開就放行。**判準:改門檻/加保護時,先分清「拖慢主因是取樣門檻(一般小怪)還是 BOSS safe(密集 BOSS 圖)」——後者才是重災區,別誤動前者。** 實測(Lv96 法師):elder_room 8 種長老 BOSS 從「全 unsafe 每次真打」→「全 safe 秒殺+補小怪」,24h 秒級算完、無死亡。
  - **⚖️ 快速結算 vs 全模擬 A/B 保真度(2026-07-08 實測,使用者已知並接受)**:同角色同圖各跑數輪比每小時金幣/經驗速率——**一般小怪圖(heine)誤差僅 +2~3%(落在 RNG 噪音內)**;**有「出怪機率+節流」的密集 BOSS 圖(elder_room)快速偏高 ~13%**(gold/exp 一致偏高、非噪音)。根因:快速段 BOSS 秒殺用「首遇對打耗時」推進、`spawnMob(0)` 連續出怪,**不重現原圖的出怪節流**(elder_room 長老 BOSS「場上最多 2 隻、第 2 隻等 3 分鐘」在快速段幾乎總放行)→ BOSS 出得比真實密 → 收益偏高;純 BOSS 圖(龍窟,固定出怪無機率節流)則兩者 BOSS 率一致、偏差小。**使用者決定接受**(偏高=多給玩家、非坑玩家;只影響有節流的少數圖;要修得在快速段重現節流、程式變複雜又與「求快」相反)。**判準:BOSS 收益占比高又有特殊出怪限制的圖,快速結算會系統性略偏高,屬已知取捨,不是 bug。**
