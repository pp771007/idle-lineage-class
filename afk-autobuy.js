 ...
 *   ② 自動購買魔法屏障卷軸    (原版只有自動「施放」、沒有自動「補貨」,卷軸用完屏障就斷)
 *
 * 設計:完全不改原作者程式碼。
 ...
 *   - 設定持久化:原版存檔只記內建的 set-* 勾選,本檔兩個勾選自行存 localStorage(跨重載記住)。
 *
...
(function () {
  ...
  var SCROLL_ID = 'scroll_magicbarrier';  // 魔法屏障卷軸:商店單買,單價 = DB.items 的 p(1500)
  var SCROLL_MIN = 3;                      // 低於這個量就補
  var SCROLL_REFILL = 5;                   // 每次補到手 5 張

  var CHECK_EVERY = 10;                    // 每 10 tick(約 1 秒)檢查一次,夠用又省

  var LS_MEAT   = 'afk_autobuy_meat';
  var LS_SCROLL = 'afk_autobuy_magicbarrier';
  ...
  function autoBuyCheck() {
    ...
    var meatChk = document.getElementById('set-auto-buy-meat');
    if (meatChk && meatChk.checked && invCount(MEAT_ID) < MEAT_MIN) {
    ...
    var sChk = document.getElementById('set-auto-buy-magicbarrier');
    if (sChk && sChk.checked && invCount(SCROLL_ID) < SCROLL_MIN) {
      ...
      if (def) {
        ...
        if (player.gold >= sCost) {
          ...
          buyLog('自動花費 ' + sCost + ' 金幣補充了 ' + SCROLL_REFILL + ' 張魔法屏障卷軸。');
        }
      }
    }
  }
  ...
  function injectUI() {
    ...
    // 還原 / 持久化勾選狀態(原版存檔不含這兩個,自存 localStorage)
    bind('set-auto-buy-meat', LS_MEAT);
    bind('set-auto-buy-magicbarrier', LS_SCROLL);
    return true;
  }

  function bind(id, lsKey) {
    var el = document.getElementById(id);
    if (!el) return;
    try { el.checked = (localStorage.getItem(lsKey) === '1'); } catch (e) {}
    el.addEventListener('change', function () {
      try { localStorage.setItem(lsKey, el.checked ? '1' : '0'); } catch (e) {}
    });
  }

  function init() {
    if (!injectUI()) console.warn('[AFK-autobuy] 找不到設定面板的「自動買銀箭」(#set-auto-buy-arrow),UI 未注入(自動購買邏輯仍會運作)。');
    console.log('[AFK-autobuy] hooks OK — 外掛自動購買(肉 / 魔法屏障卷軸)已啟用。');
  }
