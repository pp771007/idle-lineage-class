...
(function () {
  window.AFK_EXTRA = {
    ...
    skillNote: {
      ...
      sk_shock_stun: '造成一次武器物理傷害（額外 +10），命中後 10% 機率嘗試使目標暈眩 6 秒（是否暈眩成功再依等級差判定、非固定百分比；需裝備雙手武器，弓除外）',
      sk_elf_worldtree: '讓妖精森林、眠龍洞穴一帶的區域額外掉落率由 20% 提升到 30%（被動）',
      sk_elf_singleres: '提升所選定屬性的抗性 +50',
      sk_elf_magicerase: '以魔法命中判定對單體施放（本身不造成傷害）：命中後使目標「魔防減半」持續 16 秒，這 16 秒內你與傭兵對牠的魔法傷害都吃到半魔防。命中率看「施法者（用傭兵放就是傭兵）的等級＋魔法命中」對上「怪的等級＋魔防」——頭目、席琳強化怪這類高等級／高魔防目標<b>很常抵抗</b>，畫面上魔防減半一閃就沒、多半是<b>連續被牠抵抗</b>、不是效果只持續一瞬。一旦命中就是<b>完整 16 秒</b>（不是每挨一發魔法各觸發一次；效果還在時傭兵不會重複放，到期後才會再上一次）',
      ...
      sk_elf_watervital: '期間全隊（你＋所有傭兵）下次受到「治癒術」（瞬間治癒，不含持續回復）時，恢復量加倍（全隊共用一次）；觸發後 7 秒冷卻才能再次加倍。效果結束才能再施放',
      ...
      sk_illu_skullbreak: '只對「不死類」怪物有效（對其他怪會退回一般攻擊）：先做即死判定（同起死回生術——成功率看「你的等級＋魔法命中」對上「怪的等級＋魔防」、封頂約 60%、對王級無效），成功就直接秒殺。即死失敗才改造成傷害：用「粉碎能量」公式（武器骰＋近/遠距離傷害＋強化值，再乘魔法傷害加成 1＋魔法傷害÷16，不計武器特效；視為魔法故必定命中、不受目標防禦與硬皮減免），再加固定 +20，最後乘目標脆弱倍率',
      sk_illu_mindbreak: '消耗「最大 MP 的 5%」（至少 1）當作傷害打出，MP 不夠就放不出來：基礎傷害＝這次消耗掉的 MP 量，再乘魔法傷害加成（1＋魔法傷害÷16）；無屬性魔法，受目標魔防（MR）減免',
      sk_illu_cube_harmony: '開啟後每 1 秒對當前目標造成 1～25 火傷、並讓全隊（你＋所有傭兵）各回復 5 MP（整段約 20 下、回 100 MP；MP 回滿就不再回）',
