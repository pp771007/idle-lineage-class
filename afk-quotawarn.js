...
(function () {
  ...
  function ensureBanner(pct) {
    ...
    el.innerHTML = '<div style="font-weight:700;font-size:14px;color:#fca5a5;margin-bottom:4px;">⚠️ 存檔空間快滿了（已用約 ' + Math.round(pct) + '%）</div>'
      ...
      + '<br><b>先試這個</b>：到「⚙ 設定 → 檢查存檔大小」看看，若有很多<b>未壓縮</b>的存檔，'
      + '就到左上角<b>🎚️ 外掛</b>打開<b>「存檔即時壓縮」</b>，佔用能縮到約 1/10（代價是每次存檔會多花一點時間壓縮）。'
      + '<br><span style="color:#fca5a5;">還是不夠的話，再<b>刪掉幾個不玩的角色</b>釋放空間（想留下的角色可先在選角畫面點「匯出進度」存成備份檔）。</span>';
  }
