...
(function () {
  ...
  var COMBAT_SECTIONS = [
    ...
    { t: '攻擊速度：看「角色 × 武器種類」查表（每分鐘攻擊次數）', blocks: (function () {
      ...
      bl.push({ t: 'p', p: '加速類效果（加速術／勇敢藥水／精靈餅乾／切割／劍術精通／專精加成／<b>變身</b>／龍騎士覺醒…）都是在這個基礎值上<b>再乘</b>（多個一起吃就<b>互相相乘疊加</b>，不是覆蓋）；<b>變身的「攻速＋X%」也是乘在這個基礎值上、不會蓋掉職業速度</b>。負重過重的攻速懲罰見「負重」分頁。表格直接讀遊戲資料，數值會自動跟上。' });
      return bl;
    })() },
    // ── 二、你打怪：傷害怎麼算（進攻） ──
  ...
  function renderPoly() {
    ...
    h += '<div class="m-wiki-kv"><b>加成怎麼疊、攻速怎麼算</b>：變身的<b>額外傷害／命中／魔防／MP…</b>都是直接<b>加</b>在你原本數值上。<b>攻速＋X%</b>則是在你「角色（職業＋性別）×武器種類」的<b>基礎攻速上再乘</b>（<b>不是覆蓋掉職業速度</b>）——先用你配的武器查出基礎攻速，再套上變身的加成；還會與加速術／勇敢藥水／精靈餅乾／各精通<b>相乘疊加</b>。基礎攻速怎麼查、加速怎麼相乘見「能力值」分頁。</div>';
  ...
  var MERC_CLASS_ROWS = [
    { c: '🔵 法師', auto: '光箭（免費魔法普攻、不耗 MP）',
      atk: '任一<b>傷害魔法</b>（火球／冰箭／隕石…）；緩速／弱化／疾病／即死等<b>狀態魔法</b>',
  ...
  function renderAlly() {
    ...
    var c2 = card('戰力＝招募快照，但會跟你一起升級', { lines: [
      ...
      '⚙️ <b>裝備／技能／精通是定型快照</b>：事後幫那角色換裝、改精通，傭兵不會自動跟著變——要在公會點<b>「重新招募」</b>才以最新存檔更新快照。<b>重新招募比初次便宜</b>：費用＝原價（等級×10000）×曲線費率，Lv1 約 <b>1/10</b>、Lv50 約 <b>1/5</b>、Lv100 約 <b>1/2</b>，中間平滑遞增。',
      '⭐ <b>但等級會跟著你升</b>：每次擊殺，<b>每名在場傭兵各拿「自己等級的 50%」經驗</b>（不扣你的份、你照拿滿額；經典模式一樣打對折）。升級後即時重算戰力。',
      '💾 <b>賺到的經驗以「待領帳本」還給那角色</b>：解散傭兵、點「重新招募」、或你（隊長）回村時，累積經驗會記成一筆待領紀錄；<b>那名角色下次載入遊戲或回到村莊時自動一次領取</b>（可能一口氣升好幾級），借去當傭兵不算白練。',
    ...
    // 4. 隊伍面板設定
    var panelRows =
      '<tr>' + td('攻擊技能 ＋ MP&gt;門檻%') + td('選一個他學過的攻擊技當主要輸出（見上表）。攻擊技改為<b>每約 2 秒放一次、平時用普攻</b>（同你本人）。右邊 <b>MP&gt;x%</b> 是「魔力高於此才放攻擊技」，未達就先普攻回魔（0＝不限）。') + '</tr>' +
      '<tr>' + td('治癒魔法 ＋ HP&lt;門檻%') + td('隊伍（你＋傭兵們）有人 HP 低於門檻（預設 <b>70%</b>）→ 他改放治癒，補最缺血的那個。只要該角色<b>學過治癒術</b>（初／中／高／全部）就能指定。也能指定<b>吸血魔法</b>（寒冷戰慄／吸血鬼之吻）——這類只回施放者本人，所以改成<b>傭兵自己 HP 低於門檻</b>時對敵施放吸血。<b>新招募預設不放治癒、以攻擊為主</b>，要補血得在這格<b>手動指定</b>。') + '</tr>' +
      '<tr>' + td('轉換技能') + td('選一個他學過的<b>魂體轉換／心靈轉換／魔力奪取／立方：和諧</b>——用來把 HP 換 MP（或立方回魔），讓吃魔的傭兵不斷魔。受下方「停耗 HP 技」門檻限制。') + '</tr>' +
      '<tr>' + td('HP&lt;門檻% 喝藥水') + td('傭兵自己 HP 低於此→<b>喝你道具欄的藥水</b>（扣 1 瓶）回血；填 <b>0＝關閉</b>。') + '</tr>' +
      '<tr>' + td('HP&lt;門檻% 停耗 HP 技') + td('傭兵自己 HP 低於此→<b>暫停施放消耗 HP 的技能</b>（龍騎士 HP 技／轉換技能／立方和諧），退回普攻不再自殘；填 <b>0＝關閉</b>。') + '</tr>';
    var c4 = card('隊伍面板：每名傭兵各自設定', {
      html:
        tbl(th('設定') + th('作用'), panelRows) +
        desc('自動喝藥從<b>你（隊長）的道具欄</b>扣藥水，恢復量吃該傭兵自己的體質加成，約 1 秒可喝一次、硬控中也能喝。<b>你身上沒那瓶又有勾「自動購買藥水」</b>時，會自動補貨給傭兵喝。') +
        desc('資源條：<b>龍騎士</b>技能吃 <b>HP</b>、其餘有魔力的職業顯示 <b>MP</b>（騎士／戰士純物理不顯示）。<b>HP／MP 每 16 秒各自然回一些</b>（全職通用，依該角色恢復量），避免永久斷資源。')
    });
    ...
    var reviveRows =
      '<tr>' + td('🔵 返生術（手動）') + td('你學會「返生術」且 MP 足夠，於倒地卡按鈕') + td('<b>立即</b>復活、無冷卻') + '</tr>' +
      '<tr>' + td('🎫 復活卷軸（自動）') + td('該傭兵<b>倒地滿 15 秒</b>、且你身上有復活卷軸') + td('<b>自動</b>消耗 1 張復活（免手動）') + '</tr>' +
    ...
    var c5 = card('會挨打、會倒地（不是永久死）', {
      html:
        ...
        desc('<b>你自己死亡</b>後傭兵都<b>留在你身邊</b>（回城、讀檔、離線掛機回來都還在）；<b>只有</b>在公會點「⚠ 全員退出」才會解散（招募費不退還；累積經驗照樣記入待領帳本）。') +
        '<div class="m-wiki-desc" style="margin-top:6px;"><b>倒地了怎麼救回來</b>（復活卷軸已改自動；返生術在隊伍面板倒地卡上有按鈕）：</div>' +
        tbl(th('方式') + th('條件') + th('效果'), reviveRows) +
        desc('復活後 HP 回<b>一半</b>、MP <b>回滿</b>、清掉所有異常狀態。')
    });

    // 5b. 傭兵能力補完（傭兵幾乎完整發揮該角色的能力）
    var c5b = card('傭兵幾乎「照你本人的方式」打', { lines: [
      '傭兵會<b>自動維持自己的增益魔法</b>：覺醒、加速、狂暴術、神聖武器、各屬性 buff… 都會自己上（只付 MP、不扣 HP）。',
      '會<b>自動召喚</b>（造屍術／召喚術／屬性精靈，幻術士的歐吉／巫妖／鑽石高崙幻象），也會發動<b>套裝效果</b>（紅獅／狂怒／麗人／龍血／鐵衛／暗影／月光…）、<b>魔法娃娃</b>與<b>受擊反射</b>（疼痛的歡愉／致命身軀／泰坦／鏡反射）。',
      '學過<b>淨化</b>（魔法相消術／聖潔之光／解毒術）的傭兵，會<b>幫全隊解異常狀態</b>（一次解一人、優先你自己）。',
      '<b>例外：完全免疫類</b>（絕對屏障／大地屏障／魔法屏障）<b>不會</b>給傭兵自動維持（否則近乎無敵）。',
      '🌿 <b>全隊型輔助</b>（大地的祝福、鋼鐵防護、幻覺光環＝歐吉／巫妖／鑽石高崙／化身，以及生命的祝福等團隊持續回復）——<b>不論你或傭兵施放，都惠及全隊</b>（你＋所有傭兵）。'
    ]});
    ...
    return note + c1 + c2 + c3 + c4 + c5 + c5b + c6;
  }
  ...
  var MAGIC_FACT = {
    ...
    sk_lightarrow: ['吃智力(魔法傷害)、不吃力量；階級係數 ×1.33', 'MP 消耗隨智力降低(智力 45 以上最多省 30%)，不隨階級變'],
    sk_icearrow: ['吃智力(魔法傷害)、不吃力量；階級係數 ×1.33；打火屬怪 ×1.4、被風屬怪剋 ×0.6'],
    sk_windblade: ['吃智力(魔法傷害)、不吃力量；階級係數 ×1.33；打水屬怪 ×1.4、被地屬怪剋 ×0.6'],
    ...
    sk_firearrow: ['吃智力(魔法傷害)、不吃力量；階級係數 ×1.67；打地屬怪 ×1.4、被水屬怪剋 ×0.6'],
    sk_hell_fang: ['吃智力(魔法傷害)、不吃力量；階級係數 ×1.67；打風屬怪 ×1.4、被火屬怪剋 ×0.6'],
    sk_cold_shiver: ['吃智力(魔法傷害)、不吃力量；階級係數 ×1.67', '吸取與傷害等量的生命(最多回到滿血)'],
    ...
    sk_aurora: ['吃智力(魔法傷害)、不吃力量；階級係數 ×2；打水屬怪 ×1.4、被地屬怪剋 ×0.6'],
    sk_chill: ['吃智力(魔法傷害)、不吃力量；階級係數 ×2；打火屬怪 ×1.4、被風屬怪剋 ×0.6'],
    ...
    sk_fireball: ['吃智力(魔法傷害)、不吃力量；階級係數 ×2.33；打地屬怪 ×1.4、被水屬怪剋 ×0.6'],
    sk_rock_prison: ['吃智力(魔法傷害)、不吃力量；階級係數 ×2.33；打風屬怪 ×1.4、被火屬怪剋 ×0.6'],
    sk_vampire: ['吃智力(魔法傷害)、不吃力量；階級係數 ×2.33', '吸取與傷害等量的生命(最多回到滿血)'],
    ...
    sk_thunder: ['吃智力(魔法傷害)、不吃力量；階級係數 ×2.67；打水屬怪 ×1.4、被地屬怪剋 ×0.6'],
    sk_ice_spike: ['吃智力(魔法傷害)、不吃力量；階級係數 ×2.67；打火屬怪 ×1.4、被風屬怪剋 ×0.6'],
    ...
    sk_earthquake: ['吃智力(魔法傷害)、不吃力量；階級係數 ×3；打風屬怪 ×1.4、被火屬怪剋 ×0.6'],
    sk_blaze: ['吃智力(魔法傷害)、不吃力量；階級係數 ×3；打地屬怪 ×1.4、被水屬怪剋 ×0.6'],
    sk_weaken: ['本身不造成傷害；使目標命中 −2、攻擊傷害 −4、持續 30 秒，依異常魔法命中判定(等級＋魔法命中 對 怪等＋魔防)'],
    sk_haste_spell: ['戴風之頭盔時免 MP 施放'],
    sk_zombie: ['傷害命中吃魅力、不吃智力；每輪段數＝魅力÷6(魅力 60 封頂、最多 10 段)，召喚精通時判定改魅力×1.2', '每段＝(1~12)＋(魅力÷5×(1＋等級÷20))再扣目標防禦', '同時只能開一個召喚增益(造屍/召喚/精靈)，迷魅可並存'],
    sk_cancel: ['解除冰凍、石化、中毒、麻痺、灼燒、燙傷六種；一次只解一人、優先自己', '被沉默或魔法封印時放不出來'],
    sk_ice_lance: ['吃智力(魔法傷害)、不吃力量；階級係數 ×3.33；打火屬怪 ×1.4、被風屬怪剋 ×0.6', '附帶冰凍 6 秒(依異常魔法命中判定，對王級無效)'],
    sk_tornado: ['吃智力(魔法傷害)、不吃力量；階級係數 ×3.33；打水屬怪 ×1.4、被地屬怪剋 ×0.6'],
    sk_disease: ['本身不造成傷害；使目標防禦(AC) 變差 8(更好打中)、命中 −4、持續 30 秒，依異常魔法命中判定(等級＋魔法命中 對 怪等＋魔防)'],
    sk_greater_haste: ['戴風之頭盔時免 MP 施放'],
    sk_summon: ['召喚物隨等級自動換階(七種：32/40/52/60/64/72 級，Lv72 黑豹最強，60 級以上附額外觸發技)', '傷害命中吃魅力、不吃智力；每輪段數＝魅力÷6(魅力 60 封頂、最多 10 段)，召喚精通時判定改魅力×1.2', '同時只能開一個召喚增益(造屍/召喚/精靈)，迷魅可並存'],
    sk_regen: ['全隊(你＋所有傭兵)每 3 秒回 1 次、共 5 次(15 秒)，每次＝(31~60)×(1＋3×魔法傷害÷16)，吃智力'],
    sk_quake: ['吃智力(魔法傷害)、不吃力量；階級係數 ×3.67；打風屬怪 ×1.4、被火屬怪剋 ×0.6'],
    sk_blizzard: ['吃智力(魔法傷害)、不吃力量；階級係數 ×3.67；打火屬怪 ×1.4、被風屬怪剋 ×0.6'],
    ...
    sk_thunder_storm: ['吃智力(魔法傷害)、不吃力量；階級係數 ×4；打水屬怪 ×1.4、被地屬怪剋 ×0.6'],
    sk_fire_storm: ['吃智力(魔法傷害)、不吃力量；階級係數 ×4；打地屬怪 ×1.4、被水屬怪剋 ×0.6'],
    sk_meteor: ['吃智力(魔法傷害)、不吃力量；階級係數 ×4.33；打地屬怪 ×1.4、被水屬怪剋 ×0.6'],
    sk_disintegrate: ['吃智力(魔法傷害)、不吃力量；階級係數 ×4.33'],
    ...
    sk_elf_summon: ['每 1 秒攻擊一次；傷害與命中吃「魅力」不吃智力：每擊＝骰值 1~40 ＋ 魅力×等級÷20，再扣魔防與傷害減免', '隻數：無精靈精通固定 1 隻；有精靈精通＝1＋魅力÷10(最多 7 隻，魅力以 60 封頂)', '造屍術/召喚術/屬性精靈同時只能開一個，迷魅術可另外並存'],
    ...
    sk_elf_summon2: ['每 1 秒攻擊一次；每擊＝骰值 2~80 ＋ 魅力×等級÷10、命中偏移 +20(較初階精靈更強)，吃魅力不吃智力', '隻數公式同屬性精靈(1＋魅力÷10、上限 7、魅力封頂 60)；同時只能開一個召喚增益'],
    ...
    sk_dragon_lavaspit: ['傷害吃智力(魔法傷害)、完全不吃力量——純力量龍騎放這招很弱', '受火屬性剋制加減；龍血套裝 5/5 讓消耗 HP 類技能傷害 +20%、龍血精通則 HP 消耗減半'],
    ...
    sk_dragon_lavabolt: ['傷害吃智力(魔法傷害)、完全不吃力量——純力量龍騎放這招很弱', '受火屬性剋制加減、龍血套裝 5/5 傷害 +20%'],
    ...
    sk_dragon_deathlightning: ['傷害吃智力(魔法傷害)、完全不吃力量——純力量龍騎放這招很弱', '附「暈眩」6 秒依魔法命中判定、王級免疫暈眩'],
    ...
    sk_warrior_armorbody: ['本作防禦(AC)是負值、越低(越強)減免越高：防禦(AC) -40 → 減免 +5(公式無條件捨去)', '防禦(AC)為正(很弱)時算出 0、沒有效果', '學堅韌精通後除數改 5(減免加倍)：防禦(AC) -40 → 減免 +10'],
