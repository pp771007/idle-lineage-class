...
(function () {
  ...
  var COMBAT_SECTIONS = [
    ...
    { t: '硬皮：可削、會回的物理護甲', blocks: [
      ...
      { t: 'tbl', h: ['硬皮滿值（依對象）', '滿值'], rows: [
        ['一般怪', '等級的一半'], ['頭目', '等級'], ['三大龍（法利昂／安塔瑞斯／巴拉卡斯）', '等級的兩倍'], ['攻城城門', '你的等級'], ['守護塔', '你等級的一半']
      ]},
    ...
    { t: '怪打你時：迴避(ER)・傷害減免(DR)・受到傷害減少%', blocks: [
      ...
      { t: 'tbl', h: ['防禦層', '作用', '來源／規則'], rows: [
        ['① 迴避率（ER）', '有機率<b>整個閃掉（0 傷害）</b>', '主要靠<b>敏捷</b>＋buff/套裝（迴避提升+12、月光3件+5）。<b>非 1:1</b>：50 以內 1 點=1%、超過 50 每 5 點才+1%。一般只閃物理；月光5件才連魔法/必中技能也先判定'],
  ...
  function renderSherine() {
    ...
    out += wCard('🩸 席琳的世界（困難模式）',
      wDesc('到「席琳神殿」找 NPC <b>席琳</b> 祈禱開／關（需 40 級以上、可自由切換）；開啟後畫面變暗紅。<b>攻城區與血盟敵人不受影響</b>。') +
      wTbl(['項目', '開啟後'], [
        ['怪物 HP', '×3'], ['怪物 防禦／魔防', '×1.5'], ['怪物 命中', '×1.5'],
        ['怪物 額外減傷', '+（怪等÷3）'], ['怪物 一般攻擊傷害', '×2'], ['怪物 技能／持續傷害', '×2'], ['出怪等待', '−1 秒'],
        ['你的經驗', '<b style="color:#86efac">×5</b>'], ['你的金錢', '<b style="color:#86efac">×5</b>'], ['掉落機率', '<b style="color:#86efac">×3</b>'],
        ['席琳結晶／套裝', '<b style="color:#86efac">限開啟時才掉</b>']
      ]));
    ...
    out += wCard('🟢 席琳套裝加成（同名跨部位湊件數）',
      ...
      wDesc('取得帶效果裝備：席琳世界擊殺（一般怪 0.1%、恩賜怪 0.5%、BOSS 5%）、席琳製作（必帶）、席琳兌換（必帶）。') +
  ...
  function renderPledge() {
    ...
    out += wCard('💥 打血盟敵人的好處與注意',
      ...
      wDesc('另有 <b>1% 機率帶「祝福的」</b>；屬性／遠古不會隨機帶，要靠象牙塔碧恩賦予。'));
