...
(function () {
  ...
  var COMBAT_SECTIONS = [
    ...
    { t: '攻擊速度：看「角色 × 武器種類」查表（每分鐘攻擊次數）', blocks: (function () {
      ...
      bl.push({ t: 'p', p: '加速類效果（加速術／勇敢藥水／切割／劍術精通／專精加成…）都是在這個基礎值上<b>再乘</b>；負重過重的攻速懲罰見「負重」分頁。表格直接讀遊戲資料，數值會自動跟上。' });
      return bl;
    })() },
    // ── 二、你打怪：傷害怎麼算（進攻） ──
  ...
  function renderDoll() {
    ...
    out += wCard('🎁 怎麼取得（各階途徑）',
      wDesc('娃娃材料來自<b>重複卡片兌換</b>（見「收藏-怪物」分頁）：銀卡 → 魔法娃娃的袋子；金卡 → 高級魔法娃娃的盒子。開出後再用同階娃娃合成更高階。<b>娃娃沒有直接用道具「兌換」的管道</b>——高階（五階、六階）只能一路合成上去。') +
