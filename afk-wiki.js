...
(function () {
  ...
  function renderPets() {
    ...
    return note
      ...
      + wCard('② 出戰：最多 4 隻，而且要「魅力夠養」',
          ...
          + wDesc('<b>出戰是跨角色互斥</b>：一隻寵物同時只能被一個角色帶出戰；在別的角色按「轉為出戰」會把牠<b>從原角色身邊接手</b>過來。多分頁即時同步、以「最後操作者」為準（避免同一隻被兩邊雙帶、經驗錯亂）。裝備變更同理，別角色的舊快照不會洗掉你剛換的裝備。')
      ...
      + wCard('③ 戰鬥：牠自己會打，也會幫你挨打',
          ...
          + wDesc('🆕 <b>魅力就是戰力</b>：每隻出戰寵物普攻傷害與命中各 <b>+（魅力×10%，無條件捨去）</b>（有<b>王族馴養精通</b>時 12%），<b>每隻獨立、不因出戰隻數稀釋</b>；王族馴養另讓寵物<b>魔法技能傷害 +（全部魅力值）</b>。')
  ...
  var MAGIC_FACT = {
    ...
    sk_summon: ['召喚<b>多隻實體怪</b>（各有 HP、會被打死、自動重召）。共 11 階（Lv28 起每 4 級一階到 Lv72），每階數種怪；<b>沒戴召喚控制戒指</b>→固定召喚該階預設怪，戴戒指才能開選單挑其他已解鎖的怪', '隻數＝floor((魅力+6)÷該階係數)，夾在該階上限內（低階最多 5~6 隻、高階遞減、68/72 階固定 1 隻）。詳細各階怪物/攻速看遊戲選單', '傷害隨「魅力×等級」成長（同階內血少的傷害高、血多的較坦，每隻獨立成倍疊加）；召喚精通→傷害×1.2、命中+5；同時只能開一個召喚增益，迷魅可並存'],
    ...
    sk_elf_summon: ['每擊骰傷依屬性：水/風 2~76、火/地 2~66；再加固定值＝魅力×等級÷scale（水/風 ÷11、火/地 ÷12.5），扣魔防與傷害減免（魔防穿透＝10＋魅力/10）', '固定 1 隻（不隨魅力增加）；Lv40、HP 400、攻速 水/風 1.8s·火/地 1.6s；精靈有自己的 HP，會被打死、時間到或全滅自動重召', '造屍術/召喚術/屬性精靈同時只能開一個，迷魅術可另外並存'],
    ...
    sk_elf_summon2: ['骰傷依屬性：水 3~144、火 3~129、地 3~120、風 3~126；固定值＝魅力×等級÷scale（水5.5/火6.2/地6.6/風6.3）、傷害再×1.18、魔防穿透 20＋魅力/10', '固定 1 隻，Lv50、HP 600~720（風最厚）。學「精靈精通」→昇華成同屬性<b>精靈王</b>（Lv60、更硬、傷害×1.3、穿透+30，命中後 15% 釋放同屬性全體法術：水冰雪暴/火火風暴/地震裂術/風龍捲風）', '同時只能開一個召喚增益'],
