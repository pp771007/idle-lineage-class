...
(function () {
  ...
  var COMBAT_SECTIONS = [
    ...
    { t: '頭目會回血：一定要有人「用打的」', blocks: [
      ...
      { t: 'tbl', h: ['最近 5 秒內…', '每 5 秒回復'], rows: [
        ['<b>有</b>被物理攻擊命中', '最大 HP 的 <b>1%</b>'],
        ['<b>完全沒有</b>被物理攻擊命中', '最大 HP 的 <b>5%</b>']
      ]},
      { t: 'p', p: '你、<b>傭兵</b>、<b>召喚物與被迷魅的怪</b>的物理攻擊都算數；<b>魔法傷害不算</b>。所以純法術打頭目，牠會一直用 5% 的速度回血——想單靠法術磨死頭目，每秒輸出必須壓過最大 HP 的 1%。' },
    ...
    { t: '怪打你時：迴避(ER)・傷害減免(DR)・受到傷害減少%', blocks: [
      ...
      { t: 'tbl', h: ['防禦層', '作用', '來源／規則'], rows: [
        ...
        ['③ 受到傷害減少 %', '扣完 DR 後<b>整體乘算</b>（各效果相乘、不是加總）', '鐵衛3件＝×0.8（−20%）、幻覺化身 −3%、鋼鐵防護 −5%（這兩個任一隊員維持即全隊生效）、龍裔（龍血3件觸發）−15%、狂怒5件依失血最多 −20%（HP≤50%達上限）']
      ]},
  ...
  function renderPets() {
    ...
    return note
      ...
      + wCard('③ 戰鬥：牠自己會打，也會幫你挨打',
          ...
          + wDesc('經驗：把你<b>該場擊殺的經驗複製一份</b>，均分給出戰中的寵物（<b>不會扣你的經驗</b>）。寵物的升級需求是玩家的 <b>1/10</b>。')
          + wDesc('<b>寵物等級不能超過你的等級</b>。到頂的寵物不再累積經驗，牠那一份會由其他<b>還沒到頂的出戰寵物平分</b>（不會浪費）；你升級後牠就能繼續成長。')
          + wDesc('你練到 <b>Lv100</b>（自己不再獲得經驗）之後，<b>寵物仍然照常吃經驗、繼續升級</b>。'))
      + wCard('④ 裝備：每隻各自穿',
  ...
  var MERC_BUFF_SCOPE = [
    ...
    ['🌟 團隊光環（只有這 4 支）', '大地的祝福 防禦(AC) −7／鋼鐵防護 受傷 −5%／水之元氣 下次治癒 ×2／幻覺：化身 額外傷害 +10、受傷 −3%', '<b>全隊</b>：你或任一傭兵維持即可，已有人維持就不重複放'],
    ['幻覺：歐吉／巫妖／鑽石高崙', '歐吉 額外傷害+4、命中+4／巫妖 魔法傷害+2／鑽石高崙 防禦(AC) −10', '<b>只有你維持才有</b>——傭兵不會維持這三支，他只會出幻象本體']
  ];
  // 🚫 傭兵放了沒效果的自我增益（效果只寫在玩家路徑；仍會被自動維持＝白耗 MP）
  var MERC_USELESS_BUFFS = [
    ['烈焰之魂', '「近戰擲骰必最大」只寫在你的傷害公式裡，傭兵普攻走另一套'],
    ['亡命之徒', '命中保底只算在你身上'],
    ['戰斧投擲', '下一擊出血只在你的攻擊流程觸發'],
    ['毒性抵抗', '中毒減半只減你的中毒'],
  ...
  var MERC_SKILL_ROWS = [
    ...
    { c: '🍃 妖精',
      ...
      special: '<b>烈焰之魂</b>（近戰擲骰必最大）<b>無效</b>；體能激發／能量激發 對傭兵無作用（傭兵的回復不看負重）。' },
    { c: '🗡 黑暗妖精',
    ...
    { c: '🛡 戰士',
      ...
      special: '<b>戰斧投擲</b>（下一擊出血）與<b>亡命之徒</b>（命中保底）<b>都無效</b>——只幫玩家本人實作；但仍會被自動維持＝白耗 MP，可在隊伍面板關掉。' },
    { c: '👑 王族',
    ...
    { c: '🔮 幻術士',
      ...
      autocast: '<b>立方</b>（燃燒／地裂／衝擊）：要<b>來源角色有勾自動施放</b>才展開；<b>立方：和諧</b>要在隊伍面板的「<b>轉換技能</b>」欄選才展開（每秒單體 1D25 火＋全隊回 5 MP，對傭兵免費）；<b>幻覺歐吉／巫妖／鑽石高崙</b>→召喚幻象本體（需<b>幻術精通</b>）。',
      passive: '<b>奇古獸／魔劍</b>普攻（魔法傷害，奇古獸無視魔防）；<b>疼痛的歡愉</b>受擊 100% 反射（對傭兵免費）；學過並勾選的立方另給他自己火／地／風抗性 +30。',
      special: '<b>幻覺歐吉／巫妖／鑽石高崙的全隊光環，傭兵給不了</b>（他不會維持這三支）——要吃到只能<b>你自己</b>放；傭兵能提供的團隊光環只有<b>幻覺：化身</b>。幻象本體（歐吉／巫妖／鑽石高崙幫打）則需<b>幻術精通</b>才會出。' }
  ];
  ...
  function renderAlly() {
    ...
    var c6 = card('⑥ 傭兵會繼承本尊哪些能力', { lines: [
      ...
      '⚡ <b>傭兵普攻攻速與本尊同一套算法</b>：由「職業×武器」基礎攻速，再乘上他自己身上維持的全部加速類效果（加速術／強力加速／勇敢／餅乾／切割／各攻速精通／覺醒／血之渴望／變身，並吃負重懲罰）——他有勾自動施放的加速 buff 會真的讓普攻變快。',
      ...
      '🙅 <b>其餘增益都只保護「維持的那個人」</b>：傭兵的聖結界（受傷 −30%）只減他自己受的傷，你的增益也不會分給傭兵。幻覺的<b>歐吉／巫妖／鑽石高崙</b>光環只有<b>你自己</b>維持才有（傭兵不會放這三支）。<b>各職業當傭兵實際能用哪些技能、哪些放了沒用，見「傭兵可用技能」分頁。</b>'
    ]});
  ...
  function renderStats() {
    ...
    var COLS = {
      ...
      int: (typeof getIntMagicDmg === 'function') ? [
        ...
        { h: '額外MP', fx: '智力 11~59：每 +4 智力 +1（+2→+14）；60↑ 轉逐格對照（夾 +2 跳點），100 封頂 +35', f: function (v) { return sgn(getIntExtraMp(v)); } },
    ...
    return note + capSection +
      ...
      '<div class="m-wiki-desc" style="margin:8px 2px;color:#94a3b8;font-size:12px;">※ 召喚段數的「數量」以魅力 <b>60</b> 封頂（表中 70／80 與 60 相同）、帶寵上限封頂 8；傷害與命中則用完整魅力、可超過 60。<b>屬性精靈固定 1 隻</b>（不隨魅力增加）——學了「精靈精通」是讓強力屬性精靈昇華成<b>精靈王</b>，不是變多隻。</div>' +
  ...
  function renderPledge() {
    ...
    out += wCard('⚔️ 攻城戰',
      wTbl(['項目', '內容'], [
        ...
        ['自動守城（維持占領）', '每天清晨 5 點自動扣 <b>10 張王族搜索狀</b>續約，<b>不必上線</b>（關著遊戲也算，下次登入補結算）。<b>價格與你自己重新宣戰一模一樣</b>——自動化只省時間、不打折。身上不足 10 張就<b>失守</b>：8 折與城堡關閉，要重新攻城。']
      ]));

    out += wCard('🏰 城堡護衛 / 神官（攻下城池 24 小時內可雇一名）',
      wDesc('到打下那座城的城堡找「傭兵隊長／神官隊長」雇一名；<b>同時只能一名</b>、不攻擊。設 HP 門檻（預設 50%），你 HP 低於門檻才發動。<b>占領期間每天清晨 5 點自動續雇，扣一次全額雇用價</b>（＝你自己每天重雇的花費，不打折）；金幣不足時護衛離開，城池不受影響。失守或改占別座城時護衛也會解散。') +
      wTbl(['城池', '護衛類型'], [
  ...
  var MAGIC_FACT = {
    ...
    sk_elf_summon: ['每 1 秒攻擊一次；傷害與命中吃「魅力」：每擊＝骰值 1~40 ＋ 魅力×等級÷20，再扣魔防與傷害減免', '固定 1 隻（不隨魅力增加）；精靈有自己的 HP，會被怪打、打死就消失，時間到或全滅會自動重召', '造屍術/召喚術/屬性精靈同時只能開一個，迷魅術可另外並存'],
    ...
    sk_elf_summon2: ['每 1 秒攻擊一次；每擊＝骰值 2~80 ＋ 魅力×等級÷10、命中偏移 +20，吃魅力', '固定 1 隻（不隨魅力增加）。學了「精靈精通」時，這隻會昇華成同屬性的<b>精靈王</b>：等級更高、更硬、傷害更重，命中後 15% 機率釋放同屬性的全體法術', '同時只能開一個召喚增益'],
    ...
    sk_elf_steelguard: ['全隊受到傷害 −5%(乘算，你＋所有傭兵)；是減傷、不提供防禦(AC)', '傭兵也能維持：任一隊員維持即全隊生效，已有人維持時其他人不重複施放'],
