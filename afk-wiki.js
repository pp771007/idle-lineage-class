...
(function () {
  ...
  var QUEST_BY_CLASS = {
    ...
    royal: {
      ...
      trials: [
        { n: '紅色斗篷 / 魔法書(精準目標)', npc: '甘特 ＠說話之島', req: '搜索狀 ×1（需先接取試煉）', from: '搜索狀：接取甘特的 15 級試煉後，打「黑騎士搜索隊」（古魯丁）<b>必定掉落</b>，湊滿就停掉（王族限定、無法存入倉庫）', rw: '紅色斗篷（防禦(AC) -2、魅力 +1）或 魔法書(精準目標)，二選一、可重複' },
  ...
  var TRIAL_50 = {
    ...
    royal: { n: '王族 50 級試煉', npc: '迪嘉勒廷 ＠象牙塔（需等級 50）', req: '第一階段：交付 調職命令書 ×1', from: '調職命令書：小惡魔 1%（傲慢之塔 21~30 樓；王族限定）', rw: '完成後開放「魔族神殿」。在魔族神殿打「炎魔的惡魔」（Lv61）取得「炎魔之心」（3% 掉），每 1 個回來找迪嘉勒廷換傳說「黃金權杖」（王族單手劍、反擊／居合；可重複換）' },
