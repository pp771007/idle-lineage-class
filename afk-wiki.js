...
(function () {
  ...
  var ENHANCE_SECTIONS = [
    ...
    { t: '詞綴怎麼來', lines: [
      '<b>祝福</b>：打怪／製作／血盟掉落有 1% 機率直接帶「祝福的」（在「席琳的世界」擊殺時 ×3＝3%）；強化卷軸取得時也各有 1% 變成祝福／詛咒卷軸。（<b>黑市購買也一樣會擲</b>：買下的瞬間同樣有 1% 機率帶「祝福的」，席琳的世界 ×3、瘋狂的席琳世界 ×5）',
  ...
  function renderRelicMarket() {
    ...
    out += wCard('💰 他們收什麼、出多少',
      wTbl(['', '龍鑽收購者', '金幣收購者'], [
        ['收購範圍', '稀有品（黑市權重 1~10 的非遺物道具）', '一般品（黑市權重 1~80 的非遺物道具）'],
    ...
    out += wCard('📋 遺物布告欄（潘朵拉黑市）',
      ...
      wTbl(['項目', '規則'], [
        ...
        ['要交什麼', '<b>3 樣不重複的道具</b>：一樣是最稀有檔（權重 1）、一樣是權重 1~20、一樣是權重 1~50 或任何可製作的道具']
      ]) +
    ...
    out += wCard('🛒 潘朵拉黑市（陳列與收購單）',
      wTbl(['項目', '規則'], [
        ...
        ['買下時', '會<b>當場擲詞綴</b>——1% 機率帶「祝福的」（席琳的世界 ×3、瘋狂的席琳世界 ×5）']
      ]) +
      ...
      wDesc('可以指定的範圍：<b>魔法書、萬能藥，以及耳環以外的一般穿戴裝備</b>。<b>材料、支配符、遺物、魔法娃娃、寵物裝備不能指定</b>。') +
      wDesc('價格是<b>當下就定死</b>的（存讀檔重洗不掉），不能靠反覆讀檔洗到便宜價。'));
  ...
  //   何時要更新:上游改效率/上下限/可結算地圖清單/頭目遭遇率,或改動「背景分頁怎麼補」時。
  function renderOffline() {
    ...
    out += wCard('🚫 這些地方不會有離線收益',
      wTbl(['地點／狀態', '為什麼'], [
        ...
        ['<b>軍王之室</b>', '要燒鑰匙的房間，不做離線結算'],
        ['<b>純頭目房</b>', '整張圖只有頭目，沒有「一般怪速度」可以推算'],
        ['<b>傲慢之塔</b>各層（含攀登中）', '計時／闖關型，離線續跑等於刷榜'],
        ['<b>時空裂痕</b>戰鬥圖（含挑戰中）', '同上，是限時挑戰'],
        ['角色已死亡', '倒著就沒有產出']
      ]) +
      wDesc('⚠️ 這幾張圖<b>離線回來就是 0 收益</b>，不是「算比較少」。想掛整晚，關網頁前記得先移到一般狩獵圖。'));
