...
(function () {
  ...
  function dice(d) { return d[0] + 'D' + d[1]; }
  function powerTxt(sk) {
    if (sk.multiDmg) return '（威力 ' + sk.multiDmg.map(dice).join('＋') + '）';
    if (sk.dmgDice) return '（威力 ' + dice(sk.dmgDice) + '）';
    return '';
  }
  ...
  var WEAPON_TRAITS = [
    ...
    { n: '月光爆裂', d: '攻擊時 8% 機率追加一發「風屬性」固定傷害（約 1D30，再隨強化值增加），無視防禦直接打。代表武器：熾炎天使弓。' },
    ...
    { n: '命中恢復 MP', d: '普攻命中時回復 MP，強化值越高回得越多。法師持久作戰很好用。代表武器：瑪那魔杖、魔力短劍。' },
    ...
    { n: '武器附魔（傳說武器）', d: '普攻有「1% ＋ 每強化 +1%」的機率，不用學技能就免費施放武器內建的魔法。代表武器：死亡騎士的烈炎之劍、克特之劍、冰之女王魔杖、蕾雅魔杖。' },
    ...
    { n: '不死 / 狼人剋星（銀・精靈）', d: '攻擊「不死類」或「狼人」時額外 +1D20 傷害。打這兩類怪特別有效。代表武器：銀斧、精靈短劍、銀長劍，以及銀箭、米索莉箭。' },
    { n: '鈍擊（單手鈍器）', d: '命中時讓目標的下一次攻擊延遲 1 秒，拖慢敵人出手。代表武器：流星錘、釘錘、木棒。' },
    { n: '魔爆（神官魔杖）', d: '施放傷害魔法時有機率引發爆炸，對全場敵人各追加「本次魔法傷害的三成」。代表武器：神官魔杖。' }
  ];
  var WEAPON_BASICS = [
    ['小型 / 大型傷害', '武器對「小型怪」與「大型怪」各自的傷害骰；重擊時一律打出最高值。'],
  ...
  function renderWeapon() {
    ...
    return '<div class="m-wiki-note">武器上常看到的特殊效果，這裡用白話說明（強化值越高，多數效果也越強）。</div>' + traits + basics;
  }
  ...
  function renderMagic(cls) {
    ...
    var html = '<div class="m-wiki-note">' + esc(clsName) + '可學的魔法，依「可學等級」由低到高排列。' +
