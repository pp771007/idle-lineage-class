...
(function () {
  ...
  var ENHANCE_SECTIONS = [
    ...
    { t: '強化上限（+11 以上）', lines: [
      ...
      '<b>武器 +11~+20</b>：額外傷害<b>封頂在 +10 的量</b>（+11 起不再加固定傷害）；額外命中則繼續往上疊，+11~+20 的總命中約為 +11／+12／+14／+16／+18／+21／+24／+27／+31／+35。',
  ...
  function renderCard() {
    var out = '<div class="m-wiki-note">「卡片收集」：每隻怪（<b>血盟與建築類除外</b>）極低機率掉「怪物卡片」，<b>使用</b>卡片在「收集冊」登錄該怪、解鎖牠的資料；把一個地區的怪收齊還有<b>屬性加成</b>。收集冊是創角自帶、唯一、不能賣也不能存倉的道具，使用即翻開。<br><b>收集進度和倉庫同規則</b>：同一模式的角色<b>共用</b>一份——依模式組合（一般／經典／傳統／經典＋傳統，共四種）各自獨立一份，互不共通。</div>';
    ...
    out += wCard('🎴 三階卡片',
      wTbl(['卡片', '掉落機率', '解鎖的怪物資料', '自行賣價'], [
        ['普卡', '0.01%', '怪名、等級', price(1) + ' 金'],
        ['銀卡', '0.001%', '＋ HP、屬性', price(2) + ' 金'],
        ['金卡', '0.0001%', '＋ 防禦(AC)、魔防、出沒地圖', price(3) + ' 金']
      ]) +
      wDesc('三階<b>各自獨立判定</b>（同一隻可能只掉普卡、也可能直接掉金卡）。機率<b>不受經典模式 ×1/10 影響</b>（一般／經典同機率）。') +
      wDesc('撿到卡片要<b>使用</b>才會登錄；高階含低階資訊（金卡看得到全部）。<b>重複的卡片一律進背包</b>（不再自動賣），留著拿去合成、換魔法娃娃、或自己賣掉。')
    );
  ...
  function renderEquipBook() {
    ...
    var out = '<div class="m-wiki-note">「裝備收集冊」：<b>獲得任何裝備就自動登錄</b>（只增不減，賣掉／丟掉也保留紀錄），依部位分類。把<b>某部位的全部裝備</b>都收集齊，就拿到該部位的<b>永久加成</b>。收集冊是創角自帶、唯一、不能賣也不能存倉，使用即翻開。<br><b>收集進度和倉庫同規則</b>：同一模式的角色<b>共用</b>一份——依模式組合（一般／經典／傳統／經典＋傳統，共四種）各自獨立一份，互不共通。</div>';
    var groups = [];
