...
(function () {
  ...
  var WEAPON_TRAITS = [
    ...
    { n: '武器附魔（傳說武器）', d: '普攻有機率不用學技能就免費施放武器內建的魔法，而且強化值越高越容易觸發（基礎 1%，每強化 +1%）。代表武器：死亡騎士的烈炎之劍、克特之劍、冰之女王魔杖、蕾雅魔杖。<br><b>武器觸發的法術，傷害不吃法師的「法術階級加成」</b>（那個加成只有你自己耗 MP 施放時才有），所以同一招由武器自動觸發會比你親手施放弱一些；不過武器 +11 以上的最終傷害倍率仍會加成上去。' },
  ...
  var COMBAT_SECTIONS = [
    ...
    { t: '怪打你時：迴避(ER)・傷害減免(DR)・受到傷害減少%', blocks: [
      ...
      { t: 'tbl', h: ['防禦層', '作用', '來源／規則'], rows: [
        ...
        ['③ 受到傷害減少 %', '扣完 DR 後<b>整體打折（乘算）</b>', '鐵衛3件＝×0.8（−20%，物理魔法都吃）、幻覺化身 −10%']
      ]},
  ...
  var ENHANCE_SECTIONS = [
    ...
    { t: '強化上限（+11 以上）', lines: [
      ...
      '<b>武器 +11~+20</b>：額外傷害<b>封頂在 +10 的量</b>（+11 起不再加固定傷害），改成提供「<b>最終傷害倍率</b>」——+11 是 ×1.1、+12 ×1.2、……、+20 ×2.0（<b>取該階段的值、不是一直累加</b>）。這個最終傷害倍率<b>不只吃普攻——武器觸發的特效魔法與技能傷害也一起乘</b>。額外命中則繼續往上疊，+11~+20 的總命中約為 +11／+12／+14／+16／+18／+21／+24／+27／+31／+35。',
  ...
  function renderCard() {
    var out = '<div class="m-wiki-note">「卡片收集」：每隻怪（<b>血盟與建築類除外</b>）極低機率掉「怪物卡片」，<b>使用</b>卡片在「收集冊」登錄該怪、解鎖牠的資料；把一個地區的怪收齊還有<b>屬性加成</b>。收集冊是創角自帶、唯一、不能賣也不能存倉的道具，使用即翻開。<br><b>收集進度和倉庫同規則</b>：同一模式的角色<b>共用</b>一份——一般模式角色共用、經典角色另一份、傳統角色再一份（各自獨立）。</div>';
  ...
  function renderEquipBook() {
    ...
    var out = '<div class="m-wiki-note">「裝備收集冊」：<b>獲得任何裝備就自動登錄</b>（只增不減，賣掉／丟掉也保留紀錄），依部位分類。把<b>某部位的全部裝備</b>都收集齊，就拿到該部位的<b>永久加成</b>。收集冊是創角自帶、唯一、不能賣也不能存倉，使用即翻開。<br><b>收集進度和倉庫同規則</b>：同一模式的角色<b>共用</b>一份——一般模式角色共用、經典角色另一份、傳統角色再一份（各自獨立）。</div>';
    var groups = [];
  ...
  function renderSherine() {
    ...
    out += wCard('🟢 席琳套裝加成（同名跨部位湊件數）',
      wDesc('裝備可帶一個席琳套裝效果（名稱前冠套裝名，如「紅獅環甲」）；<b>同一套裝名戴在不同部位</b>累計件數，湊 2／3／5 件解鎖各階。可裝：武器／頭盔／盔甲／手套／長靴／斗篷／腰帶。') +
      wDesc('取得帶效果裝備：席琳世界擊殺（一般怪 0.1%、恩賜怪 0.5%、BOSS 5%；<b>瘋狂席琳再 ×3</b>）、席琳製作（必帶）、席琳兌換（必帶）。') +
      wTbl(['套裝', '2 件', '3 件', '5 件'], setRows));

    out += wCard('✨ 席琳恩賜（精英怪）',
      wDesc('席琳世界每次刷怪 1% 機率（每 3 分鐘最多一次），隨機一隻普通怪變「恩賜精英」（紅綠光暈＋紅底綠字徽章）；牠更兇但報酬更高。') +
  ...
  function renderMode() {
    ...
    var c5 = card('🎴 卡片 · 裝備收集冊', tbl(
      r('卡片掉率', '100%', '<b style="color:#86efac">同一般（不吃 ×1/10）</b>', '<b style="color:#86efac">同一般</b>') +
      r('收集冊進度共用範圍', '一般角色互通', '經典角色互通', '傳統角色互通')
    ) + '<div class="m-wiki-desc" style="margin-top:6px;">・卡片／裝備兩本收集冊都<b>跟倉庫同規則</b>：同一模式的角色共用一份、三種模式各自獨立（切到別的模式＝另一份進度）。詳細玩法見「卡片」「裝備圖鑑」分頁。</div>');
