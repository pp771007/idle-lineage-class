...
const _PM_ATTACK_SKILLS = { sk_dragon_slaughter: true, sk_elf_triple: true, sk_warrior_throwaxe: true };   // 玩家端：以技能 id 判斷·次數讀 DB.skills[id].hits
const _PM_ATTACK_SKILL_NAMES = { '屠宰者': 3, '三重矢': 3 };   // 傭兵端：以技能名判斷(js/06 傳 sk.n)·值=連擊次數
function _vfxLayer() {
...
//   （上游掛點在 doTeleport/enterHiddenArea；本 fork 尚未接掛點,_teleportFxUntil 恆 0＝sprite 不隱藏,函式保留供之後接。）
...
function _battleSpriteProbe(form) {
    ...
    let finish = () => { if (--pending <= 0) { _morphBattleCache[form.key] = out; _battleSpriteEvict(); } };
...
function _playerMorphApply() {   // 8fps ticker 驅動（🗡️ v3.0.67 形態＝變身優先→職業動態·key 含武器變體→換武器自動重建）
    ...
    if (typeof _partyBottoms !== 'undefined' && _partyBottoms && _partyBottoms.P != null) { _pmState.el.style.bottom = _partyBottoms.P + 'px'; _pmState.el.style.zIndex = String(Math.max(2, 30 - _partyBottoms.P)); }
...
function _allySpriteTrigger(ally, k, skId) {   // js/06 掛點：allyAttackOnce→'attack'·三施法函式→'skill'
    try {
        ...
        if (k === 'skill' && _PM_ATTACK_SKILL_NAMES[skId]) {   // 🗡️ 屠宰者/三重矢：改播攻擊動畫連擊（取代施法動作）
...
function _allySpritesApply() {   // 8fps ticker 驅動（先於 _playerMorphApply→_partyBottoms 供主玩家消費）
    ...
    allies.forEach((ally, i) => {
        ...
        if (st.key !== form.key) { if (st.el) { try { st.el.remove(); } catch (e) {} } st.el = null; st.imgs = null; st.act = null; st.key = form.key; st.pendAtk = false; st.atkBurst = false; st.atkRepeat = 0; }   // 🔮 v3.0.96 換形態→清排隊攻擊
        ...
        if (_partyBottoms && _partyBottoms[slot] != null) { st.el.style.bottom = _partyBottoms[slot] + 'px'; st.el.style.zIndex = String(Math.max(2, 30 - _partyBottoms[slot])); }   // z 下限 2 → 永遠在怪物層(z:1)之上,避免大隊伍後排傭兵被怪物蓋掉(30-bottom 會為負)
