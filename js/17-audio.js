...
// 🔇 發聲總閘：玩家開關 && 非快轉。離線/背景補跑(state.ff)會逐拍跑完 24h≈86 萬拍，
//    逐拍播擊殺/受擊音既洗耳朵又白吃效能 → 這裡第一行就擋掉（連音檔懶載/查表都不做）。
//    設定 UI(set-sfx-on / setSfxOn)仍直接讀寫 _sfxCfg.on 的玩家真值。
function _sfxAudible() { return _sfxCfg.on && !state.ff; }
// 🗡️ v3.0.111 玩家武器攻擊音改用「天堂正宗揮擊音」（依武器類型→ list.spr `attack <武器>` 動作的攻擊音編號·檔＝assets/sfx/<n>）。
...
function playMobAttack(mob) {
    if (!_sfxAudible() || !mob) return;   // 🔇 _sfxAudible 含 state.ff：離線補跑逐拍觸發怪物攻擊·一律靜音
...
function playMobSkill(mob) {
    if (!_sfxAudible() || !mob) return;   // 🔇 _sfxAudible 含 state.ff：離線補跑期間靜音
