...
// 🩸 owner 參數化（預設玩家·傭兵召喚術傳 ally）：選怪/數量/傷害/命中皆依 owner 的等級·魅力·召喚控制戒指。
...
function _zmbTierForPlayer(owner) {
    owner = owner || player;   // 🩸 owner 參數化（傭兵造屍術傳 ally）
...
// 🧙 傭兵召喚術 v2 抽象輸出計畫：依傭兵（owner）等級＋召喚控制戒指選怪·回 { form, count, lv }（純供傷害計算·不生成戰場實體/血條/面板）；無法召喚回 null（呼叫端落回舊分階模型）。
...
function summonV2AttackOnce(s, d, t, owner) {
    owner = owner || player;   // 🩸 owner 參數化：傭兵召喚術抽象輸出共用（讀 owner 裝備/精通；killMob 仍歸真隊長·不換身）
